...
//
//   noswoosh            daemon: Ctrl+Left/Right switches spaces instantly;
//                       trackpad swipes retain native macOS animation
...
// distance to travel, so it is instant. Ctrl+arrow and the CLI use this core.
// Real trackpad gestures are left to macOS, preserving native animation and
// interactive tracking. Posting events requires Accessibility permission.
//
...
// Keep synthetic event tags for compatibility with an older daemon that may
// still intercept gestures. This daemon does not install a gesture event tap.
...
// MARK: - Switch core (hotkeys and CLI)
...
// Ctrl+Left / Ctrl+Right hotkey (the only intercepted input).
...
// No gesture event tap: macOS handles real trackpad swipes unchanged.
